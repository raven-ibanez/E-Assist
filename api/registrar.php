...
// Load email helper functions for sending status notifications
...
if ($action === 'review_application') {
    ...
    if ($db_decision === 'approved') {
        $emailDecision = $docs_pending ? 'approved_dtf' : 'approved';
    } elseif ($db_decision === 'declined') {
...
if ($action === 'review_payment') {
    ...
    $stmtCheck = $conn->prepare("SELECT decision FROM enrollment_reviews WHERE enrollment_id = ? AND review_type = 'Cashier' ORDER BY created_at DESC LIMIT 1");
    $stmtCheck->bind_param("i", $enrollment_id);
    $stmtCheck->execute();
    $currentStatusRow = $stmtCheck->get_result()->fetch_assoc();
    $currentStatus = $currentStatusRow['decision'] ?? 'pending';

    if ($decision === 'declined' && $currentStatus === 'declined') {
        sendJSON(['error' => 'This payment is already declined.'], 400);
    }
...
if ($action === 'refund_excess') {
    ...
    $totalPaid = $payInfo['total_paid'] ?? 0;
    $totalFee  = $payInfo['total_fee'] ?? 0;
    
    // IF application is declined, refund the WHOLE paid amount
    if ($payInfo['registrar_status'] === 'declined') {
        $excess = $totalPaid;
        $notes = "Full student payment refund (Declined Enrollment)";
    } else {
        $excess = $totalPaid - $totalFee;
...
// =============================================================
//  ACTION: REFUND STUDENT PAYMENT (Cashier refund full payment)
//  - Used when cashier approved but registrar declined
// =============================================================
if ($action === 'refund_student') {
    ...
    if ($total_paid <= 0) {
        sendJSON(['error' => 'No payment found to refund.'], 400);
    }

    // Check if it has a refund already to prevent double refunding
    $stmtCheck = $conn->prepare("SELECT SUM(amount_paid) as refunded_amount FROM payment_transactions WHERE payment_id = ? AND amount_paid < 0");
    $stmtCheck->bind_param("i", $payInfo['payment_id']);
    $stmtCheck->execute();
    $refCheck = $stmtCheck->get_result()->fetch_assoc();
    $refunded = abs($refCheck['refunded_amount'] ?? 0);
    
    // We want to refund the remaining unrefunded amount
    $to_refund = $total_paid - $refunded;

    if ($to_refund <= 0) {
        sendJSON(['error' => 'Payment has already been refunded.'], 400);
    }
    ...
    sendJSON(['message' => 'Student payment refunded successfully.']);
}
...
// =============================================================
//  ACTION: GET DELETED STUDENTS
//  - Returns a list of all deleted enrollment applications (status='deleted' only).
// =============================================================
if ($action === 'deleted_students') {
    $result = $conn->query("
        SELECT 
            e.id AS enrollment_id, 
            e.applied_at,
    ...
    if (!$result) {
        sendJSON(['error' => 'Failed to fetch deleted students: ' . $conn->error], 500);
    }
...
// =============================================================
//  ACTION: GET DELETED EMPLOYEES
//  - Returns a list of deleted employee accounts (status='deleted' only).
// =============================================================
if ($action === 'deleted_employees') {
    ...
    if (!$result) {
        sendJSON(['error' => 'Failed to fetch deleted employees: ' . $conn->error], 500);
    }
...
// =============================================================
//  ACTION: RESTORE STUDENT FROM DELETE
...
// =============================================================
//  ACTION: RESTORE EMPLOYEE FROM DELETE
//  - Restores a deleted employee record back to active status.
// =============================================================
if ($action === 'restore_employee_from_deleted') {
    $id       = $data['id'] ?? '';
    $admin_id = $data['admin_id'] ?? '';
    ...
    $stmt = $conn->prepare("UPDATE admin SET status = 'active' WHERE id = ? AND status = 'deleted'");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    ...
    if ($stmt->affected_rows > 0) {
        ...
        logAction($admin_id, "Restore Employee From Delete", $id, $employeeName, "Employee record restored from deleted status to active.");
        sendJSON(['message' => 'Employee record restored successfully.']);
    } else {
